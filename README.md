...
1. **`GiddhDSCBridge-Setup-1.1.0.exe`** — the Windows installer.
2. **`dsc-bridge/extension/`** — the browser extension folder.

### Windows (no admin required)

1. **Run the installer** `GiddhDSCBridge-Setup-1.1.0.exe`. It is **unsigned**, so
...
1. **Open** `GiddhDSCBridge-1.1.0.dmg`, then run the `.pkg` inside. It is
...
2. Build + install the OS installer:
   ```bash
   VERSION=1.1.0 ./packaging/macos/build_macos.sh
   open dist/GiddhDSCBridge-1.1.0.dmg      # run the .pkg (right-click → Open; it is unsigned)
   ```
...
```bash
git tag v1.1.0 && git push origin v1.1.0   # or run it manually from the Actions tab
```
